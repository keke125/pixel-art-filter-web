...
public class UserProfileView extends VerticalLayout
     ...
     */
    public UserProfileView(UserService service,
        ...
        title = new H3(translator.getTranslation("user-profile-update",
                UI.getCurrent().getLocale()));

        passwordField1 = new PasswordField(translator.getTranslation("Old" +
                "-password", UI.getCurrent().getLocale()));
        passwordField2 = new PasswordField(translator.getTranslation("New" +
                "-password", UI.getCurrent().getLocale()));
        passwordField3 = new PasswordField(translator.getTranslation("New" +
                "-password-again", UI.getCurrent().getLocale()));

        Span errorMessage = new Span();

        submitButton = new Button(translator.getTranslation("Update-profile",
                UI.getCurrent().getLocale()));
        ...
        // VerticalLayout.
        FormLayout formLayout = new FormLayout(title, passwordField1,
                passwordField2, passwordField3,
        ...
        binder.forField(passwordField1).asRequired().withValidator
                (this::matchedPasswordValidator).bind("hashedPassword");
    ...
    private void showSuccess(User user) {
        Notification notification =
                Notification.show(String.format(translator.getTranslation(
                                "profile-updated", UI.getCurrent().getLocale()),
                        user.getName()));
     ...
     */
    private ValidationResult matchedPasswordValidator(String pass1,
                                                      ValueContext ctx) {
        ...
        return ValidationResult.error(translator.getTranslation("Password" +
                "-dont-match-with-old", UI.getCurrent().getLocale()));
    }
    ...
    public void localeChange(LocaleChangeEvent localeChangeEvent) {
        title.setText(translator.getTranslation("user-profile-update",
                UI.getCurrent().getLocale()));
        passwordField1.setLabel(translator.getTranslation("Old-password",
                UI.getCurrent().getLocale()));
        passwordField2.setLabel(translator.getTranslation("New-password",
                UI.getCurrent().getLocale()));
        passwordField3.setLabel(translator.getTranslation("New-password-again"
                , UI.getCurrent().getLocale()));
        submitButton.setText(translator.getTranslation("Update-profile",
                UI.getCurrent().getLocale()));
    }
