...
public class GalleryViewCard extends ListItem implements LocaleChangeObserver {
    ...
    public GalleryViewCard(ImageService imageService, ImageInfo imageInfo, User user) {
        ...
        imagesTabs.add(translator.getTranslation("Original", UI.getCurrent().getLocale()), new Div(new Image(originalResource, "original-" + imageInfo.getUploadImageName())));
        imagesTabs.add(translator.getTranslation("Generated", UI.getCurrent().getLocale()), new Div(new Image(generatedResource, "generated-" + imageInfo.getUploadImageName())));
        ...
        // close detail dialog
        closeButton = new Button(translator.getTranslation("Close", UI.getCurrent().getLocale()), e -> detailDialog.close());
        closeButton.addThemeVariants(ButtonVariant.LUMO_TERTIARY);
        detailDialog.getHeader().add(closeButton);


        // delete image button
        deleteButton = new Button(translator.getTranslation("Delete", UI.getCurrent().getLocale()));
    ...
    public void localeChange(LocaleChangeEvent localeChangeEvent) {
        detailDialog.setHeaderTitle(translator.getTranslation("Image-Info", UI.getCurrent().getLocale()));
        detailButton.setText(translator.getTranslation("View-Details", UI.getCurrent().getLocale()));
        imagesTabs.getTabAt(0).setLabel(translator.getTranslation("Original", UI.getCurrent().getLocale()));
        imagesTabs.getTabAt(1).setLabel(translator.getTranslation("Generated", UI.getCurrent().getLocale()));
        colorNumber.setText(translator.getTranslation("Color-number", UI.getCurrent().getLocale()) + ": " + imageInfo.getColorNumber());
        pixelSize.setText(translator.getTranslation("Pixel-size", UI.getCurrent().getLocale()) + ": " + imageInfo.getPixelSize());
        if (getLocale().equals(Translator.LOCALE_ZHT)) {
            smooth.setText(translator.getTranslation("Smooth", UI.getCurrent().getLocale()) + ": " + PixelTransformView.Smooth.valueToName(imageInfo.getSmooth(), "TC"));
            edgeCrispening.setText(translator.getTranslation("Edge-crispening", UI.getCurrent().getLocale()) + ": " + PixelTransformView.EdgeCrispening.valueToName(imageInfo.getEdgeCrispening(), "TC"));
        } else {
            smooth.setText(translator.getTranslation("Smooth", UI.getCurrent().getLocale()) + ": " + PixelTransformView.Smooth.valueToName(imageInfo.getSmooth(), "EN"));
            edgeCrispening.setText(translator.getTranslation("Edge-crispening", UI.getCurrent().getLocale()) + ": " + PixelTransformView.EdgeCrispening.valueToName(imageInfo.getEdgeCrispening(), "EN"));
        }
        saturation.setText(translator.getTranslation("Saturation", UI.getCurrent().getLocale()) + ": " + imageInfo.getSaturation());
        contrastRatio.setText(translator.getTranslation("Contras-ratio", UI.getCurrent().getLocale()) + ": " + imageInfo.getContrastRatio());
        parameterDetails.setSummaryText(translator.getTranslation("Image-transform-parameter", UI.getCurrent().getLocale()));
        downloadOriginal.removeAll();
        downloadOriginal.add(new Button(translator.getTranslation("Download-Original", UI.getCurrent().getLocale())));
        detailDialog.getFooter().add(downloadOriginal);
        downloadGenerated.removeAll();
        downloadGenerated.add(new Button(translator.getTranslation("Download-Generated", UI.getCurrent().getLocale())));
        detailDialog.getFooter().add(downloadGenerated);
        deleteButton.setText(translator.getTranslation("Delete", UI.getCurrent().getLocale()));
        closeButton.setText(translator.getTranslation("Close", UI.getCurrent().getLocale()));
    }
}
